...
from arcface_r50 import ArcFace
...
@app.post("/face/log/jetson_nano")
async def log_faces(item: Miris_raw):
    # print("TIME: ", datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    print("UPLOAD TIME: ", item.time)
    print("RECEIVE TIME: ", time.time())
    # process raw faces into unique faces
    ...
    for i in range(len(item.raw_faces)):
        ...
        raw_faces.append(face_cv)
        item.raw_faces[i]['face'] = face_cv
        cb = item.raw_faces[i]['crop_box']
        cb = [int(b) for b in cb]
        print(cb, raw_faces[-1].shape)
        item.raw_faces[i]['face_crop'] = face_cv[cb[1]:cb[3], cb[0]:cb[2], :]
    ...
    for i in range(len(raw_faces)):
        ...
        # cb = [int(b) for b in cb]
        # print(cb)
        raw_faces_center.append(item.raw_faces[i]['face_crop'])
        cv2.imwrite(f"images/debug/jetson/{i}.jpg", raw_faces_center[-1])
        cv2.imwrite(f"images/debug/jetson_full/{i}.jpg", raw_faces[i])
